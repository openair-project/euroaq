...
- `import_eea_stations(countries = "FR")` accesses data flow D (station metadata). Note that one or more countries must be specified.
...
- `import_eea_cities(countries = "FR")` accesses city names in a given country or countries.

Each of these functions will return R data frames, making them convenient for further analysis and filtering.

To actually download data, you may use `import_eea_monitoring()`. This function has four main arguments:
...
4. `dataset`, requiring a value of either `1` (unratified, up-to-date data from data flow E2a), `2` (ratified data from data flow E1a), `3` (Historical Airbase data), `4`, (Downscaled CAMS forecast at station level) or `5` (Gap-filled E2a, using linear interpolation for smaller gaps, and a regressor based on similar stations for larger ones).
...
Modelled data at each monitoring station can be accessed using `dataset = 4` in the already discussed functions. Additionally, the EEA Download Portal gives access to complete forecast maps of downscaled CAMS data, for entire countries or capital cities, in raster format. There are two ways to access this:

- Most conveniently, a user can use `import_eea_raster()` which downloads the requested raster files and reads them into R. This required the `{terra}` package.

- For more flexibility, lower-level API functions are provided - `get_eea_raster_files()` and `get_eea_raster_async()`. These work similarly to their `get_eea_parquet_files()` and `get_eea_parquet_async()` equivalents, already discussed.
...
Note that there is a limit to the width of the date range of data accessed in one time using this approach. The user must also specify a single country and at least one pollutant.
